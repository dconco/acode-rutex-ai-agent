...
import { RANDOM_RESPONSES } from './panel/constants'
import { renderMarkdown } from './panel/markdown'
import type { AIPanelAPI, ChatMessage, ContextFile, EditorManagerLike, FileLike } from './panel/types'
import { decodeBase64Safe, escapeHtml, getElement, getFileNameFromPath } from './panel/utils'
...
ctxAddBtn.onclick = event => openContextMenu(event.currentTarget as HTMLElement)
attachBtn.onclick = event => openContextMenu(event.currentTarget as HTMLElement)
...
const editorManager = acode?.require?.('editorManager') as EditorManagerLike | undefined
...
const content =
file.content || file.session?.getValue?.() || file.editor?.getValue?.() || ''
...
messages.forEach((message, index) => msgsInner.appendChild(buildRow(message, index)))
scrollBottom()
}
...
row.innerHTML = `
     ...
     <div class="user-bubble">
       ${
msg.ctxName
? `<div class="user-ctx-chip"><svg viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>${esc(msg.ctxName)}</div>`
: ''
}
       ${esc(msg.text)}
     </div>
...
const thumbDownBtn = row.querySelector<HTMLButtonElement>('.thumbdn-btn')
...
function simulateAIResponse(_userMessage: string, ctxName: string | null): void {
...
liveContent = liveRow.querySelector<HTMLElement>('#live-ai-content')
}
...
renderMarkdown(aiText, esc) + '<span class="stream-cursor"></span>'
attachCodeButtons(liveContent)
}
scrollBottom()
chunkIndex += 1
streamTimeout = window.setTimeout(sendChunk, 30 + Math.random() * 50)
} else {
endStream()
}
}
...
const ctxName = ctxFiles.length ? ctxFiles.map(file => file.name).join(', ') : null
