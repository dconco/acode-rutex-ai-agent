...
try {
const models = (
require(`../chats/models/${provider}_models`).default ?? []
...
if (!modelMeta) {
infoEl.innerHTML = '<span class="model-info-empty">No metadata available for this model.</span>'
return
}
...
infoEl.innerHTML = `
<div class="model-info-title">${escapeHtml(modelMeta.label)}</div>
...
modelOpenAITrigger.textContent = getModelLabel('openai', aiSettings.models.openai)
modelDeepSeekTrigger.textContent = getModelLabel(
'deepseek',
aiSettings.models.deepseek
)
modelClaudeTrigger.textContent = getModelLabel('claude', aiSettings.models.claude)
modelGeminiTrigger.textContent = getModelLabel('gemini', aiSettings.models.gemini)
